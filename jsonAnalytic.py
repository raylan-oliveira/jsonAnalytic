...
def criar_json(name_file_json, dados):
	name_file_json = name_file_json[:50]
	name_file_json = name_file_json.replace('/', '_')
	name_file_json = name_file_json.replace('\\', '_')
	if create_cache_global == 'true':
		...
		if exists(name_file) == False:
			...
			print(f'\n[{name_file}] exist... Not created!')

from time import sleep
from os import mkdir

def print_key(key):
	if regex_key_global != 'false':
		regex_key_local = regex_key_global
		regex_key_local = regex_key_local.replace('{key}', '\\1')
		regex = re.compile(r'(^.+?$)')
		regex_result = regex.sub(rf'{regex_key_local}', key)
		print(f'{regex_result}', end='')
	else:
		print(key)

def print_value(value):
	if regex_value_global != 'false':
		regex_value_local = regex_value_global
		regex_value_local = regex_value_local.replace('{value}', '\\1')
		regex = re.compile(r'(^.+?$)')
		regex_result = regex.sub(rf'{regex_value_local}', value)
		print(f'{regex_result}', end='')
		...
		for key in test_isinstance.keys():
			...
			if all_keys_or_keys_search == 'all_keys':
				...
				if dict_armazenar not in list_all:
					...
					print_value(test_isinstance[key])
...
if __name__ == "__main__":
	global regex_key_global, regex_value_global
	regex_key_global = 'false'
	regex_value_global = 'false'
	...
	if '-p' in sys.argv or '-P' in sys.argv or '-pipe' in sys.argv or '--PIPE' in sys.argv:
		...
		parser.add_argument("-p", "--pipe", action="store_true", help='Using pipe/stdin')
		parser.add_argument('-k', '--keys_search', help='keys search', required=False, type=str, default=None,  nargs='+')
		parser.add_argument('-c' , '--create_cache', help='Create *_cache.json default="true"', required=False, type=str, default='true')

		parser.add_argument('-rk', '--regex_key', help='RegEx in KEY; \\1 or {key}', type=str, default='false')

		parser.add_argument('-rv', '--regex_value', help='RegEx in VALUE; \\1 or {value}', type=str, default='false')
		...
		regex_key_global = args.regex_key
		regex_value_global = args.regex_value
		...
		if re.search(r'^http', args.input_):
			...
			url_encode = args.input_.encode('utf-8')
			js = requests.get(url_encode, headers=headers)
